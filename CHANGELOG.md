...
### Added — Quant 3.0 plans 01-05

- **Persistent session-aware market data (plan 01).** Durable market state moved out of the renderer into a main-process cache under `market-cache-v3`, with gzip payloads, a hashed manifest, atomic temp-and-rename writes, oldest-first pruning to a 512 MiB budget, and orphan sweeping. Retrieval is cache-first with a documented precedence: a stale *live* payload beats a synthetic one and is flagged `stale`, while sample data is never persisted and is refused on read. Candles carry a `session` (`pre`/`regular`/`post`/`closed`), classified through `Intl.DateTimeFormat` so DST comes from the IANA database rather than a hand-maintained offset table. Renderer refresh follows reported market state and correctly treats `PREPRE`/`POSTPOST` as dead zones.
- **Chart workspace, events and immutable signal history (plan 02).** A provider-neutral chart-event boundary over pure parsers for the Fed calendar, BLS and BEA schedules, Treasury auctions and earnings — all fixture-tested offline, with a bundled FOMC fallback marked `provenance: 'sample'`. Event reaction is benchmark-relative and explicitly non-causal. Signal history is append-only: a stored snapshot shows what the model said then, and only the forward `outcome` block may be filled in later.
- **Personal portfolio, risk and exposure (plan 03).** Local-only portfolio with whole-document validation, atomic writes and backup recovery that never seeds a fabricated position. Risk uses date intersection rather than forward-fill, empirical (not parametric) VaR and CVaR, and carries cash as a zero-volatility sleeve so it visibly dilutes risk. Fund look-through is explicitly a floor with `coveragePercent` stated. CSV import previews every row it will not import, with its reason.
- **Full-market discovery and attention (plan 04).** Replaces `universe.slice(0, limit)`-as-"US stocks" with a hydrated-universe scan that reports its whole funnel. Features use robust median/MAD z-scores and beta-adjusted residuals; attention evidence is grouped and capped so correlated views of one move cannot compound; novelty scores change rather than strength.
- **QRM-3 research pipeline, benchmark and Lab (plan 05).** Point-in-time state estimation, leakage-safe analogue records, ESS-targeted kernel bandwidth, a seeded stationary block bootstrap, and a walk-forward harness that scores CRPS and direction Brier against three simple baselines before evaluating the promotion gates. QRM stays experimental; runtime is a product gate that never justifies weakening a statistical one.
